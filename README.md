...
Cada ambiente individual tem seu **próprio catálogo de itens** (`ITENS_POR_AMBIENTE`) — "Varanda" e "Varanda Gourmet" são duas listas completamente diferentes, mesmo os dois sendo áreas externas: Varanda tem Piso/Guarda-Corpo/Fechamento de Vidro; Varanda Gourmet tem Bancada/Churrasqueira/Pia/Coifa; Quintal tem Portão/Gramado/Muro/Iluminação Externa. `getCatalogoItensDoAmbiente(nome)` busca a lista certa (com fallback pro catálogo genérico de 8 itens — `ITENS_PADRAO_GENERICO` — quando o ambiente é personalizado ou não bate com nenhuma chave conhecida, ex.: "Quarto 2"). Ao criar um ambiente, o hook já insere os itens desse catálogo automaticamente — igual ao comportamento anterior, só que agora a lista muda de verdade dependendo do ambiente escolhido.
