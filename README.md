...
- **Nível 2 — Itens do ambiente**: ao tocar em "Vistoriar Ambiente", a tela troca para a lista de itens daquele ambiente — cada um com seletor de **Condição** (`Ótimo`/`Bom`/`Regular`/`Ruim`), seletor de **Funcionamento** (`Sim`/`Não`, útil para eletros/eletrônicos), campo de observação e upload de foto **por item**, com limite de **30 fotos por item** (contador "X/30" sempre visível). O card "Adicionar item" tem um campo "Pesquise aqui..." que filtra o catálogo do ambiente atual (excluindo itens já adicionados) e mostra o resto como pílulas clicáveis — cada uma com seu próprio "+"; o mesmo campo também aceita texto livre pra um item totalmente personalizado, via o botão "Adicionar" ao lado. **← Voltar para Lista de Ambientes** retorna ao Nível 1.

### Catálogo de ambientes e itens

Nada de lista única de 6 ambientes genéricos com os mesmos 12 itens pra todos — o seletor "Adicionar ambiente" (Nível 1) agora tem **37 ambientes individuais**, agrupados em `<optgroup>` por categoria (`AMBIENTES_GRUPOS`, em `src/lib/vistoriaExecucao.js`): Ambientes Sociais, Ambientes Privativos, Cozinha e Serviços, Banheiros, Áreas Externas e Lazer, Comerciais e Apoio (a última opção desse grupo, "Outro Ambiente (Personalizado)", libera um campo de texto livre — é o antigo "Outro").

Cada ambiente individual tem seu **próprio catálogo de itens** (`ITENS_POR_AMBIENTE`) — "Varanda" e "Varanda Gourmet" são duas listas completamente diferentes, mesmo os dois sendo áreas externas: Varanda tem Piso/Guarda-Corpo/Fechamento de Vidro; Varanda Gourmet tem Bancada/Churrasqueira/Pia/Coifa; Quintal tem Portão/Gramado/Muro/Iluminação Externa. `getCatalogoItensDoAmbiente(nome)` busca a lista certa (com fallback pro catálogo genérico de 8 itens — `ITENS_PADRAO_GENERICO` — quando o ambiente é personalizado ou não bate com nenhuma chave conhecida, ex.: "Quarto 2").

**Ambiente nasce vazio, sem pré-carregar item nenhum**: ao criar um ambiente, `addAmbiente` no hook não insere mais o catálogo automaticamente — `vistoria_itens` começa como `[]`. O catálogo (`getCatalogoItensDoAmbiente`) só é usado como **sugestão** na busca "Adicionar item" dentro do ambiente (ver acima) — o vistoriador escolhe cada item, um a um, sob demanda, pela busca/pílulas ou por texto livre. Isso é uma mudança de comportamento em relação a uma entrega anterior, onde os 8-12 itens do catálogo já vinham criados automaticamente; agora o ambiente abre limpo, só com a barra de busca, e uma mensagem "Nenhum item adicionado ainda" enquanto estiver vazio.
...
### Bug real: fotos/carimbos sobrepostos no rodapé da página (removido `minPresenceAhead`)

Você mandou um laudo real onde, bem no rodapé de uma página, várias miniaturas e badges de data/hora de itens diferentes apareciam **sobrepostos uns aos outros** — carimbos com horários diferentes ("10:45:41", "10:45:42", "10:45:43") empilhados no mesmo lugar, junto com a legenda "Teto" por cima de tudo. Investigando, o suspeito mais forte era o `minPresenceAhead` que eu tinha adicionado numa entrega anterior (no título do ambiente e no nome de cada item) como reforço extra contra título órfão — é uma prop menos comum do `@react-pdf/renderer`, e empilhá-la em muitos elementos perto de uma quebra de página parece ter disparado um bug de layout mais sério do que o problema original que ela tentava resolver.

**A correção**: removi `minPresenceAhead` dos dois lugares. `wrap={false}` sozinho (no bloco de texto do item, e em cada linha de fotos — ambos já corrigidos em entregas anteriores) já resolve o problema original (título órfão, foto cortada) sem esse efeito colateral. Também reforcei o item 3 do pedido — `overflow: 'hidden'` no `photoFrame` (o container direto da imagem no PDF; a versão HTML já tinha `overflow-hidden` desde a entrega da legenda) — como camada extra de proteção pra o carimbo nunca vazar visualmente da miniatura, mesmo num cenário de layout inesperado.

**Sobre o HTML não precisar da mesma correção**: o bug de sobreposição é específico do motor de paginação do `@react-pdf/renderer` (que decide sozinho onde cortar página, com suas próprias regras de layout) — a versão HTML usa o fluxo normal de documento do navegador, sem esse tipo de decisão automática de quebra (só entra em jogo quando o usuário imprime via Ctrl+P, e ali já usamos `break-inside-avoid` nos lugares certos desde entregas anteriores). Não havia `position: absolute`/`float` fora de controle no HTML — só o badge do carimbo, que já ficava contido corretamente.
...
- **Fotos em 4:3 exato**: 160×120pt no PDF, 128×96px no HTML.

**Bug real encontrado e corrigido — título de ambiente/item pulando pra outra página com muito espaço em branco sobrando**: analisei um laudo gerado (anexado na entrega) e confirmei visualmente — "1. Recepção" ficava sozinho no rodapé de uma página, com "1.1 Iluminação" inteiro pulando pra próxima; o mesmo se repetia em "1.3 Piso", "1.4 Parede" etc., cada um começando página nova mesmo com mais da metade da página anterior vazia. A causa: meu `<Text>` de Condição continha um `<Text>` **aninhado** dentro dele (pro trecho em negrito colorido) — texto aninhado ("rich text") é um ponto conhecido de imprecisão no cálculo de altura do `@react-pdf/renderer`, e isso fazia o motor de paginação errar a estimativa de "esse bloco cabe aqui?" e empurrar o bloco `wrap={false}` inteiro pra página seguinte, mesmo sobrando espaço de sobra. Corrigi trocando por `<Text>` **irmãos** (Condição e Funcionamento como elementos de texto separados, não aninhados um dentro do outro) e reforcei com `minPresenceAhead` — uma prop oficial do `@react-pdf/renderer` feita exatamente pra evitar título órfão, aplicada no título do ambiente (`minPresenceAhead={40}`) e no nome de cada item (`minPresenceAhead={20}`): garante que sempre haja um mínimo de conteúdo seguinte "presente" antes de permitir uma quebra de página natural naquele ponto.
- Continua valendo a regra de antes: só o **bloco de texto** (título + nome + condição + observação) tem essa proteção contra quebra; a **grade de fotos** fica de fora, podendo fluir pra página seguinte livremente quando não houver espaço — exatamente como pedido.

**Sobre o carimbo de data/hora não aparecer nas fotos desse laudo específico — não é um bug de renderização**: audite o código com cuidado antes de mexer, e a causa é outra. `coletarFotosDoItem()` combina fotos vindas do join com `vistoria_fotos` (que têm `created_at` de verdade) com URLs presentes só em `item.fotos_urls` sem linha correspondente em `vistoria_fotos` — e essas últimas recebem `created_at: null` explicitamente, porque a gente genuinamente não sabe quando aquela foto foi tirada (ela só sobreviveu como URL solta, sem o registro completo). Como o carimbo só renderiza quando há uma data real (`{carimbo && (...)}`), fotos sem `created_at` não recebem carimbo nenhum — de propósito, pra não inventar um horário que não existe. Se as 57 fotos dessa vistoria específica vieram todas por esse caminho (dados de teste anteriores a algumas correções desta conversa), nenhuma delas teria carimbo, mesmo com o código correto. Não fabriquei uma data falsa pra "resolver" isso — geraria uma informação enganosa num documento que pode virar prova/laudo formal.
...
Depois de finalizar a vistoria, o vistoriador vê dois botões no card "Esta vistoria já foi encerrada": **"Imprimir / Baixar Laudo PDF"** (baixa uma cópia local, como já existia) e **"Sincronizar Vistoria"** — ambos ficam lado a lado.

"Sincronizar Vistoria" gera o laudo, sobe pro Storage, grava `laudo_pdf_url` e marca `vistorias.sincronizado = true` (e agora também `status: 'finalizada'` explicitamente, como reforço — ver seção anterior); ao terminar, mostra a mensagem de sucesso "Vistoria sincronizada com sucesso! O laudo já está disponível para o solicitante." (`sincronizarVistoria()` em `useVistoriaExecucao.js`). Diferente de `salvarLaudoPdf` (melhor esforço, nunca lança), esta função **lança em caso de erro** — é a ação principal do botão, então uma falha precisa aparecer de verdade, não ser engolida silenciosamente. Se a vistoria já foi sincronizada antes, um aviso "✓ Esta vistoria já foi sincronizada" aparece mesmo sem clicar de novo.
